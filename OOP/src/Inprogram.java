...
public class Inprogram extends JPanel implements Runnable {
    ...
    public Inprogram() {
        ...
        setBackgroundImage();
        loadMeteorSprites();
        new Thread(this, "MeteorLoop").start();
    }
    ...
    private void explode(int index) {
        ...
        String expPath = System.getProperty("user.dir")
                + File.separator + "image" + File.separator + "explosion.gif";
        ...
        while (alive) {
            if (meteor != null) {
                for (int i = 0; i < count; i++) {
                    ...
                    if (meteorX[i] <= 0) {
                        meteorX[i] = 0;
                        speedX[i] = Math.abs(speedX[i]) * boost;
                    ...
                    if (meteorY[i] <= 0) {
                        meteorY[i] = 0;
                        speedY[i] = Math.abs(speedY[i]) * boost;
                    } else if (meteorY[i] >= H - sz) {
                        meteorY[i] = H - sz;
                        speedY[i] = -Math.abs(speedY[i]) * boost;
                    }

                    if (speedX[i] > 8)  speedX[i] = 8;
                    if (speedY[i] > 8)  speedY[i] = 8;
